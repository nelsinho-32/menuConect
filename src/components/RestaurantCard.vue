<template>
    <div @click="$emit('viewRestaurant', restaurant)"
        class="restaurant-card bg-gray-900 rounded-2xl shadow-lg overflow-hidden group cursor-pointer relative h-80">
        ...
        <button @click.stop.prevent="$emit('toggleFavorite', restaurant)"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16">
                <path v-if="isFavorited"
                    d="M3.612 15.443c-.343.197-.734-.044-.657-.449l.75-4.226-3.06-2.925c-.28-.266-.125-.715.225-.76l4.24-.607 1.89-3.834c.175-.354.63-.354.805 0l1.89 3.834 4.24.607c.35.045.504.494.225.76l-3.06 2.925.75 4.226c.077.405-.314.646-.657.449L8 13.197l-4.388 2.246z" />
                <path v-else
                ...
                <div class="mt-2 flex items-center text-xs text-white/80">
                    ...
                    <div v-if="distanceInKm" class="flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor"
                            viewBox="0 0 16 16">
                            <path
                                d="M.5 1a.5.5 0 0 0 0 1h1.11l.401 1.604 1.498 7.985A.5.5 0 0 0 4 12h1a2 2 0 1 0 0 4 2 2 0 0 0 0-4h7a2 2 0 1 0 0 4 2 2 0 0 0 0-4h1a.5.5 0 0 0 .491-.408l1.5-8A.5.5 0 0 0 14.5 3H2.89l-.405-1.621A.5.5 0 0 0 2 1H.5zM6 14a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm7 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM9 5.5V7h1.5a.5.5 0 0 1 0 1H9v1.5a.5.5 0 0 1-1 0V8H6.5a.5.5 0 0 1 0-1H8V5.5a.5.5 0 0 1 1 0z" />
                        </svg>
                        <span>~{{ distanceInKm }} km</span>
                    </div>
                </div>
                <div v-if="restaurant.review_count > 0"
                    class="mt-2 flex items-center gap-1.5 text-xs text-yellow-300 font-bold">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor"
                        viewBox="0 0 16 16">
                        <path
                            d="M3.612 15.443c-.343.197-.734-.044-.657-.449l.75-4.226-3.06-2.925c-.28-.266-.125-.715.225-.76l4.24-.607 1.89-3.834c.175-.354.63-.354.805 0l1.89 3.834 4.24.607c.35.045.504.494.225.76l-3.06 2.925.75 4.226c.077.405-.314.646-.657.449L8 13.197l-4.388 2.246z" />
                    </svg>
                    <span>{{ parseFloat(restaurant.average_rating).toFixed(1) }}</span>
