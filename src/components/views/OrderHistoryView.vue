            ...
            <div v-for="order in orderStore.orderHistory" :key="order.id"
                ...
                <div v-if="order.encontro_id" class="flex items-center gap-2 mb-4 pb-4 border-b border-indigo-200">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-indigo-600">Organizado e pago no dia {{ formatDate(order.created_at) }}.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row justify-between sm:items-start border-b pb-4 mb-4">
                    ...
                    <div class="text-left sm:text-right mt-2 sm:mt-0">
                        <p class="font-bold text-xl text-indigo-600">R$ {{
                            parseFloat(order.total_price).toFixed(2).replace('.', ',') }}</p>
                        ...
                        <div v-for="(item, index) in order.items" :key="index" class="flex gap-4">
                            ...
                            <div class="flex-grow">
                                <div class="flex justify-between">
                                    <span class="font-semibold text-gray-800">{{ item.quantity }}x {{ item.dishName
                                        }}</span>
                                    <span class="font-semibold text-gray-800">R$ {{ (parseFloat(item.price_at_time) *
                                        item.quantity).toFixed(2).replace('.', ',') }}</span>
                                </div>
                                <div v-if="item.customization && item.customization.removedIngredients && item.customization.removedIngredients.length > 0"
                                    class="text-xs text-red-600 mt-1">
                                    <span>Sem: {{ item.customization.removedIngredients.join(', ') }}</span>
                                </div>
                                <div v-if="item.customization && item.customization.notes"
...
defineEmits(['backToMain', 'reorder', 'trackOrder']);
