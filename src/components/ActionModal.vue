    ...
    <div @click.self="$emit('close-modal')" class="fixed inset-0 modal-backdrop flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-2xl max-w-xs w-full shadow-2xl p-6 text-center relative">
            ...
            <div class="flex flex-col gap-3">
                <button @click="$emit('add-to-cart', { dish, quantity })" class="w-full bg-indigo-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-700 transition-colors">Adicionar ao Carrinho</button>
                <button @click="$emit('order-now', { dish, quantity })" class="w-full bg-gray-200 text-gray-700 px-6 py-3 rounded-lg font-bold hover:bg-gray-300 transition-colors">Pedir Agora</button>
            </div>
        </div>
    </div>
</template>
...
defineEmits(['close-modal', 'add-to-cart', 'order-now']);
