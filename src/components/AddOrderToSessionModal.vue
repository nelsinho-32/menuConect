        ...
        <div class="bg-white rounded-2xl max-w-5xl w-full shadow-2xl relative max-h-[90vh] flex flex-col">
            ...
            <div class="flex-grow grid grid-cols-1 md:grid-cols-2 overflow-hidden">
                <div class="p-6 border-r overflow-y-auto">
                    <div v-for="category in Object.keys(categorizedMenu)" :key="category" class="mb-4">
                        ...
                        <div class="space-y-2">
                            <div v-for="item in categorizedMenu[category]" :key="item.id" @click="addItemToOrder(item)"
                                class="p-2 rounded-lg hover:bg-gray-100 cursor-pointer flex justify-between items-center">
                                <span>{{ item.dishName }}</span>
                                <span class="font-semibold text-sm text-gray-600">R$ {{
                                    parseFloat(item.price).toFixed(2).replace('.', ',') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="p-6 bg-gray-50 flex flex-col">
                    ...
                    <div class="flex border-b mb-4">
                        <button v-for="(guest, index) in guests" :key="guest.id || index"
                            @click="activeGuestId = guest.id || index"
                    ...
                    <div class="flex-grow space-y-3 overflow-y-auto pr-2">
                        <p v-if="activeGuestOrder.length === 0" class="text-center text-gray-400 mt-10">Selecione um
                            item do cardápio para {{ activeGuestName }}.</p>

                        <div v-for="item in activeGuestOrder" :key="item.uniqueId"
                            class="bg-white p-2 rounded-md shadow-sm">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="font-semibold">{{ item.dishName }}</p>
                                    <p class="text-sm text-gray-500">R$ {{
                                        parseFloat(item.price).toFixed(2).replace('.', ',') }}</p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <button @click="$emit('customizeItem', item)" title="Personalizar"
                                    ...
                                    <button @click="removeItem(item.uniqueId)"
                                        class="w-6 h-6 rounded-full bg-red-100 text-red-600 hover:bg-red-200 font-bold">-</button>
                                </div>
                            </div>
                            <div v-if="item.customization" class="text-xs mt-1 pl-1 border-l-2 border-indigo-200">
                                <p v-if="item.customization.removedIngredients?.length" class="text-red-500">Sem: {{
                                    item.customization.removedIngredients.join(', ') }}</p>
                                <p v-if="item.customization.notes" class="text-gray-500">Nota: {{
                                    item.customization.notes }}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="p-4 bg-gray-100 border-t flex justify-end gap-3">
                <button @click="$emit('close')"
                    class="bg-white border border-gray-300 text-gray-700 font-bold py-2 px-6 rounded-lg hover:bg-gray-50">Cancelar</button>
                <button @click="handleSubmit" :disabled="totalOrder === 0"
                    class="bg-green-500 text-white font-bold py-2 px-6 rounded-lg hover:bg-green-600 disabled:bg-gray-300">Confirmar
                    Pedido</button>
            </div>
        </div>
    </div>
